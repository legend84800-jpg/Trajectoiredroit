...
(function () {
  ...
  function build() {
    ...
    // Décalage calculé sur la vraie hauteur de la barre (pas un forfait fixe) : sur mobile
    // étroit, la barre peut faire plus de 92px si son contenu tient sur plusieurs lignes,
    // et un forfait fixe laissait alors le bouton chevaucher son dernier bouton (trouvé le
    // 22/09/2026, C18 de l'audit mobile).
    var stickyBar = document.querySelector(".sticky-cta-bar");
    if (stickyBar) {
      var syncFabOffset = function () {
        var visible = getComputedStyle(stickyBar).display !== "none";
        fab.classList.toggle("tjd-fab--raised", visible);
        fab.style.bottom = visible ? (stickyBar.getBoundingClientRect().height + 32) + "px" : "";
      };
      syncFabOffset();
      window.addEventListener("resize", syncFabOffset);
      if (typeof ResizeObserver === "function") {
        new ResizeObserver(syncFabOffset).observe(stickyBar);
      }
    }
