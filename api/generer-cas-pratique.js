const { PROMPT_METHODE } = require("./_methode-cas-pratique.js");
...
module.exports = async (req, res) => {
  ...
  const enonce = typeof corps.enonce === "string" ? corps.enonce.trim() : "";

  if (enonce.length < MIN_CHARS) {
    res.status(422).json({ erreur: "L'énoncé est trop court. Colle le texte complet du cas pratique." });
    return;
  }
  if (enonce.length > MAX_CHARS) {
    res.status(413).json({ erreur: "L'énoncé est trop long. Colle un seul cas pratique (environ 20 000 caractères au maximum)." });
    return;
  }
  ...
  try {
    ...
    if (!texte || texte.startsWith("ERREUR_PAS_UN_ENONCE")) {
      res.status(422).json({ erreur: "Ce texte ne ressemble pas à un énoncé de cas pratique. Colle le texte complet de l'exercice." });
      return;
    }
